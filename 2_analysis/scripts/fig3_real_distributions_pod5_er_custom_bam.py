...
Hybrid script: end reasons from POD5 files, read length and Q-score from
a user-supplied BAM file. Use this when the BAM you want to plot was produced
by a different basecalling run than the one whose raw POD5 files you have
(e.g. a re-basecalled BAM with a newer model).

End reasons come from POD5 read_table (authoritative, not dependent on
dorado version). Read length (query_length) and Q-score (qs tag) come from
the provided BAM file. The two are joined by read_id.
...
Usage:
    python fig3_real_distributions_pod5_er_custom_bam.py \\
        --pod5-dir /nfs/turbo/.../20250519_1041_MN48328_AYJ384_c3faa658/pod5 \\
        --bam      /path/to/rebasecalled.bam \\
...
def build_dataset(pod5_dir: Path, bam_file: Path) -> pd.DataFrame:
    """Join POD5 end reasons with BAM metrics from the provided BAM file."""
    print(f"\nLoading POD5 end reasons from {pod5_dir}", flush=True)
    ...
    if len(bam) and unmatched / len(bam) > 0.05:
        raise ValueError(
            f"{unmatched:,}/{len(bam):,} BAM reads had no matching POD5 end reason (>5%). "
            "Are the POD5 files from the same flow cell as the BAM?"
        )
    print(f"\nJoined: {len(merged):,} reads ({unmatched:,} BAM reads unmatched)", flush=True)
...
def make_figure(data: pd.DataFrame, peaks: list[int], out_dir: Path) -> None:
    set_plot_style()
...
def write_lineage(out_dir: Path, pod5_dir: Path, bam_file: Path,
                  peaks: list[int], data: pd.DataFrame) -> None:
    focus = data[data["end_reason"].isin(FOCUS_CLASSES)]
    lineage = {
        "figure_id": "fig3_real_distributions",
        "paper_id": "end-reason",
        "generated_at": datetime.now(timezone.utc).isoformat(timespec="seconds"),
        "script": "2_analysis/scripts/fig3_real_distributions_pod5_er_custom_bam.py",
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--pod5-dir", type=Path, required=True,
        metavar="DIR",
        help="Directory containing *.pod5 files for the run (source of end reasons).",
    )
    ap.add_argument(
        "--bam", type=Path, required=True,
        metavar="BAM",
        help="BAM file to take read lengths and Q-scores from.",
    )
    ap.add_argument(
        "--out-dir", type=Path,
...
def main() -> int:
    ...
    if not pod5_dir.exists():
        print(f"ERROR: POD5 directory not found: {pod5_dir}", flush=True)
        return 1
    if not bam_file.exists():
        print(f"ERROR: BAM file not found: {bam_file}", flush=True)
        return 1

    print(f"POD5 directory  : {pod5_dir}", flush=True)
    print(f"BAM file        : {bam_file}", flush=True)
    print(f"Output directory: {out_dir}", flush=True)
    print(f"Expected peaks  : {peaks if peaks else '(none)'}", flush=True)

    data = build_dataset(pod5_dir, bam_file)
    make_figure(data, peaks, out_dir)
    write_lineage(out_dir, pod5_dir, bam_file, peaks, data)
